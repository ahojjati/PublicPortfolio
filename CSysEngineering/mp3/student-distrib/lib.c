...
{
    if(c == '\n' || c == '\r') {
        curr_terminal->screen_y++;
        curr_terminal->screen_x=0;
    } else {
        *(uint8_t *)(video_mem + ((NUM_COLS*curr_terminal->screen_y + curr_terminal->screen_x) << 1)) = c;
        *(uint8_t *)(video_mem + ((NUM_COLS*curr_terminal->screen_y + curr_terminal->screen_x) << 1) + 1) = ATTRIB;
        curr_terminal->screen_x++;
        curr_terminal->screen_x %= NUM_COLS;
        curr_terminal->screen_y = (curr_terminal->screen_y + (curr_terminal->screen_x / NUM_COLS)) % NUM_ROWS;
    }
}
