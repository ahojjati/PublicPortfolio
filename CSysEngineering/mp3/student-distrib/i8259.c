...
{
	//enable an interrupt on the master
	if (irq_num < MASTER_INTERRUPTS)
	{
		master_mask = inb(MASTER_DATA) & ~(1 << irq_num);
		outb(master_mask, MASTER_DATA);
	}
	// enable an interrupt on the slave
	else
	{
		irq_num -= 8;
...
{
	// disable an interrupt on the master
	if (irq_num < MASTER_INTERRUPTS)
	{
		master_mask = inb(MASTER_DATA) | (1 << irq_num);
		outb(master_mask, MASTER_DATA);
	}
	// disable an interrupt on the slave
	else
	{
		irq_num -= MASTER_INTERRUPTS;
		slave_mask = inb(SLAVE_DATA) | (1 << irq_num);
		outb(slave_mask, SLAVE_DATA);
	}
